...
3. 実測値(23.363s / 9.109s / 3.270s)を、条件依存を踏まえて正しく解釈できる。
...
結論を一言で言えば、この条件では `FAST` で約2.56倍、`MT` で約7.15倍の短縮例になっている。  
次に見るべきポイントは「なぜそう見えるか」である。`FAST` は主に反復の呼び出しコスト削減が効き、`MT` は盤面を分割して計算資源を並列利用できるため、条件が合えば差が大きくなる。  
ただし最重要なのは、これを絶対値として断定しないことだ。あくまで共有された実行環境での結果であり、CPU、ビルド設定、実行条件の違いで値は変わる。再測定を行う場合は、サイズ・回数・Release/Debug を必ず併記して比較可能性を担保する。
