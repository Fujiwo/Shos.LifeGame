...
![LifeGame](Images/lifefame01.20240112.161130.gif)

![LifeGame](Images/lifegame01.png)

- [YouTube](https://youtu.be/ny-VKZEAVkg)
...
![図1 Shos.LifeGame 実行画面イメージ](./Images/ShosLifeGame_Figure01_Overview.svg)
※ 実際の実行画面とは異なる

### 最適化の結果図3はこの責務分担を1枚で示した地図として使ってほしい。
...
実行時には `MainWindow::Next` が1ステップの起点となり、`game.Next()` で盤面を進めた直後に再描画して、世代数とFPS (ここでは1秒当たりの世代数) をタイトルへ反映する。
...
先に全体像を掴むために、4要素の比較を1表で置いておく。`BoardPainter` は描画最適化として別枠で扱う。

| 要素 | 内容 | 主に効く場所 | 効きやすい条件 | 主な副作用 |
|---|---|---|---|---|
| USEBITS | セルを1bitで保持 | セルデータ保持/更新/描画 | ビット表現を維持したい場合 | FALSE時はメモリ増・変換負荷増 |
...
|---:|:---:|:---:|:---:|:---:|---:|---:|---:|
| 0 | FALSE | FALSE | FALSE | FALSE | 63.8s | 16 | 1.00 |
| 1 | TRUE | FALSE | FALSE | FALSE | 60.2s | 17 | 1.06 |
| 2 | TRUE | TRUE | FALSE | FALSE | 24.2s | 41 | 2.56 |
| 3 | TRUE | TRUE | TRUE | FALSE | 11.7s | 86 | 5.38 |
| 4 | TRUE | TRUE | TRUE | TRUE | 4.1s | 244 | 15.25 |
...
IDE で、`USEBITS` / `FAST` / `MT` / `AREA` のON/OFFを行うことで、そうしたトレードオフ検証を行うことができるだろう。
