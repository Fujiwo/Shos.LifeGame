...
2. `Shos.LifeGame` は4要素(`USEBITS` / `FAST` / `MT` / `AREA`)でボトルネックを分解し、`BoardPainter` は描画最適化として別枠で扱う。  
...
2) **Shos.LifeGameの設計と最適化:4要素と描画最適化で追う実装読解**  
3) **LifeGameはなぜ速くなるのか:Shos.LifeGameで学ぶ実装と検証**

---

## 4. 到達目標(読了後にできること)
- 4要素(`USEBITS` / `FAST` / `MT` / `AREA`)と描画最適化(`BoardPainter`)の役割を、実装箇所付きで説明できる。
...
| 4章 | 「更新処理と描画処理を一本の線で捉えると、ボトルネックが見える。」 | 「次章では、この線上のどこを4要素と描画最適化で速くするのかを分解する。」 |
| 5章 | 「速くする手段は4要素と描画最適化に分かれ、効く場所と副作用はそれぞれ違う。」 | 「次章では、実測値を“結論先出し"で確認し、読み解き方を整理する。」 |
...
- 第5章: 4要素と描画最適化を「効く場所」で整理し、暗記でなく構造理解へ導く。
...
| 図8 最適化マップ | 5章 | 4要素と描画最適化はどこに効くか? | レイヤー別効果を明記 |
| 図9 処理方式比較 | 5章 | 面積限定/並列の差は? | 比較観点(面積・並列)を明記 |
| 図10 ベンチ結果 | 6章 | どこまで速くなったか? | 条件(サイズ/回数/環境依存)を明記 |

---

## 8. 高速化4要素+描画最適化の解説テンプレート(統一フォーマット)
### 8-0. 章冒頭の比較表(先出し)
- 第5章の冒頭に、4要素の比較表(効く条件 / 効きにくい条件 / 主な副作用)を置く。`BoardPainter` は描画最適化として別枠で補足する。
...
- 本実装での基本方針: 速度重視なら `USEBITS` は TRUE(定義)を基本とする。
- 主効果: 実装単純化の可能性。
...
4. 第5章で4要素をテンプレートで統一説明し、描画最適化を別枠で説明する。
